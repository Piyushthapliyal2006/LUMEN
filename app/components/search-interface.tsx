        ...
        {showSpaces ? (
          <section className="flex-1 overflow-y-auto px-8 py-10">
            <div className="mx-auto max-w-5xl">
              ...
              {openSpace ? <div><button type="button" onClick={() => setOpenSpace(null)} className="mb-4 rounded-md border border-border px-3 py-2 text-sm hover:bg-accent">Back to spaces</button><div className="rounded-xl border border-border bg-card p-6"><h2 className="text-xl font-semibold">{openSpace}</h2><p className="mt-1 text-sm text-muted-foreground">Saved conversations</p><div className="mt-6 space-y-2">{(spaceItems[openSpace] ?? []).map((item) => <button key={item} type="button" onClick={() => { const selected = history.find((entry) => entry.title === item); if (selected) { handleSelectHistory(selected); setShowSpaces(false) } }} className="block w-full rounded-md bg-muted px-3 py-2 text-left text-sm hover:bg-accent">{item}</button>)}</div></div></div> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{spaceNames.map((name) => <article key={name} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const title = event.dataTransfer.getData("text/lumen-history"); if (title && !spaceItems[name]?.includes(title)) setSpaceItems((current) => ({ ...current, [name]: [...(current[name] ?? []), title] })) }} className="min-h-40 rounded-xl border border-border bg-card p-4"><div className="flex items-start justify-between gap-3"><h2 className="font-medium">{name}</h2><button type="button" aria-label={`Delete ${name}`} onClick={() => { setSpaceNames((current) => current.filter((item) => item !== name)); setSpaceItems((current) => { const next = { ...current }; delete next[name]; return next }) }} className="text-xs text-muted-foreground hover:text-destructive">Delete</button></div><div className="mt-5 space-y-1">{(spaceItems[name] ?? []).map((item) => <div key={item} className="truncate rounded bg-muted px-2 py-1 text-xs">{item}</div>)}</div><button type="button" onClick={() => setOpenSpace(name)} className="mt-6 w-full rounded-md border border-border py-2 text-sm text-muted-foreground hover:bg-accent">Open</button></article>)}</div>}
            </div>
          </section>
        ) : (
