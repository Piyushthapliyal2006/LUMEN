...
export function AiComparison({ providers, messages }: { providers: Provider[]; messages: Message[] }) {
  ...
  useEffect(() => {
    ...
    visible.forEach((provider) => {
      if (submittedQueries.current[provider] !== latestQuery) void submit(provider, latestQuery)
    })
  }, [latestQuery, visible])
  ...
  return <section className="flex min-h-0 flex-1 flex-col gap-3 overflow-auto p-3 md:flex-row md:overflow-hidden" aria-label="AI comparison workspace">
    {visible.map((provider) => <div key={provider} className="relative flex min-h-[280px] min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-sm" style={{ flexBasis: `${widths[provider] || 100 / visible.length}%` }}>
